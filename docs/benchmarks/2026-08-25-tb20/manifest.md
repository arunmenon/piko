...
The headline figures are the HEADLINE line of the generated block below
(measured spend per solve, solves, priced-trial coverage), self-reported by
the runtime at real cached rates. Nothing numeric on this page is
hand-maintained: bench/tb20_manifest.py renders the block, and CI runs its
--check mode so a changed results.json cannot leave this page stale.

## Score, honestly accounted

<!-- GENERATED by bench/tb20_manifest.py from results.json; do not edit by hand -->
HEADLINE (ADR 0017 fitness function): $0.795 measured spend per solve; 33/89 solved (37.1%); $26.24 measured across 71 priced trials.
Trials: 89. Solved: 33/89 (37.1%).
Measured spend: $26.24 across 71 priced trials (18 unpriced, so true spend is higher).
Total measured spend per solve: $0.795 (subtotal over priced trials, not a rate).
Ceiling stops in all: 28 (25 unsolved, 3 solved while being cut off).

| Class | Count | Attribution |
|---|---|---|
| Solved | 33 | outcome |
| Scored but failed | 11 | capability |
| Stopped by the per-trial spend cap (unsolved) | 25 | cost-bounding choice; counterfactual unknown |
| Agent or verifier timeout | 7 | time-bounding choice; counterfactual unknown |
| Docker / install / other harness failures | 13 | local infrastructure; counterfactual unknown |
| Other errors | 0 | unclassified |

Cost distributions (priced trials only):

| Class | n priced | min | median | max |
|---|---|---|---|---|
| Solved | 33 | $0.043 | $0.258 | $0.668 |
| Ceiling-stopped (incl. solved-while-capped) | 28 | $0.243 | $0.543 | $0.800 |
| Scored failures | 11 | $0.112 | $0.273 | $0.683 |
<!-- END GENERATED -->


33/89 solved (37.1%) with every planned trial in the denominator.
Decomposition: see the generated block above; bench/tb20_manifest.py is the single source for every count on this page.
...
longer horizons, and a different (harder) task set. 25 of our failures are
...
retry the 13 infrastructure failures) is the obvious next measurement; its budget is derived by the
pre-registered protocol in docs/maturity-plan-2026-09.md (T3), which
withdraws the earlier $15-30 estimate as underived. Remaining credit at run end:
roughly $5, insufficient; a top-up decision is the owner's.

## The ceiling clips inside the solve distribution (measured)

Cost distributions: see the generated block above (per-class n, min, median, max).
...
priced trial $1.50 nominal / ~$0.70-1.0 actual, and the 25 unsolved stops are
recorded as failures, not excluded. Infra failures (13) are counted
